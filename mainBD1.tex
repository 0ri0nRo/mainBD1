...
\[Z_{0}, Z_{1}, \dots, Z_{f}, \dots \]
\[S_{0}, S_{1}, \dots, S_{i}, \dots\]

Indichiamo con $Z_{0}$ il valore iniiale di $Z (Z_{0} = X)$ e con $Z_{f}$ e $S_{i}$, $i \geq 1$ i valori di $Z$ ed $S$ dopo l'$i$-esima esecuzione del corpo del ciclo, infatti notiamo che $Z_{f} \subseteq Z_{i+1}$, per ogni $i$.\par
...
\begin{itemize}
  ...
  \end{itemize}
  \pagebreak
  \item Dimostriamo ora che $X^{+} \subseteq Z_{f}$:\par
  \begin{itemize}
    \item Sia $X \subseteq R$ e sia $r$ istanza di $R(Z_{f}, R-Z_{f})$. 
    \begin{table}[ht]
      \centering
      \begin{tabular}{|c|c|c|c|c|c|}
      \hline
      \multicolumn{3}{|c|}{$Z_{f}$} & \multicolumn{3}{c|}{$R - Z_{f}$} \\
      \hline
  ...
  \begin{itemize}
    \item $t_{1}[Z_{f}] = (1, \dots, 1) = t_{2}[Z_{f}]$
    \item $t_{1}[R - Z_{f}] = (1, \dots, 1) \neq (0, \dots, 0) = t_{2}[R - Z_{f}]$
  \end{itemize}
...
Sia $R$ uno schema e sia $F$ un insieme di dipendenze funzionali definite su $R$. Definiamo il sottoinsieme di attributi $K \subseteq R$ come chiave di $R$ se:
\begin{itemize}
