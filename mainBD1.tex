...
Sia R uno schema con decomposizione $\rho$ = $R_1, \cdots, R_k$ e sia F un insieme di dipendenze funzionali su R.
Definiamo $\rho$ come una buona decomposizione di R se:
\begin{itemize}
  \item Ogni sottoschema $R_1, \cdots, R_k$ $\in$ $\rho$ è in \textbf{Terza Forma Normale}
  \item $\rho$ permette di mantenere soddisfatta ogni dipendenza in $F$ per ogni istanza legale $r$ di $R$ ricostruita attraverso un join naturale tra tutte le istanze $R_1, \cdots, R_k$ rispettivamente di $R_1, \cdots, R_k$ \textbf{(preservazione di $F$)}
  \item $\rho$ permette di ricostruire senza perdita di informazioni le tuple di ogni istanza legale r di R ricostruita attraverso un join naturale tra tutte le istanze $R_1, \cdots, R_k$ rispettivamente di $R_1, \cdots, R_k$ \textbf{(join senza perdita)}
\end{itemize}
...
\[ \text{RpB =} \biggl\lceil \frac{BS \cdot 0.5}{RS} \biggr\rceil \]
